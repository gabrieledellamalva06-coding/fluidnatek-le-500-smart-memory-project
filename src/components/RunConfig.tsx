...
import type { ProcessConditionKey, ProcessConditionQuery } from "../features/experimental-assistant/processConditionSimilarity.types";
...
import { buildCharacterizationReview, buildFormulationReview, buildOperatingChanges, buildSetupReview, compactRows, OPERATING_PARAMETER_META, type ChangeSource, type OperatingValues, type ParameterTrace } from "../features/experimental-assistant/runReview";
...
export default function RunConfig({
  ...
  const [initialOperatingValues, setInitialOperatingValues] = useState<OperatingValues>({ flowRateMlH: 1, voltageKv: 15, collectorVoltageKv: 0, temperatureC: 25, humidityPct: 40, distanceMm: 150, drumSpeedRpm: 0 });
  const [parameterTraces, setParameterTraces] = useState<Partial<Record<RecommendedParameterKey, ParameterTrace>>>({});
  ...
  const [executedProcessQuery, setExecutedProcessQuery] = useState<ProcessConditionQuery | null>(null);
  ...
  const parameterSetters: Record<RecommendedParameterKey, (value: number | undefined) => void> = { flowRateMlH: setFlowRateMlH, voltageKv: setVoltageKv, collectorVoltageKv: setCollectorVoltageKv, temperatureC: setTemperatureC, humidityPct: setHumidityPct, distanceMm: setDistanceMm, drumSpeedRpm: setDrumSpeedRpm };
  const updateOperatingParameter = (key: RecommendedParameterKey, value: number | undefined) => {
    parameterSetters[key](value);
    setParameterTraces((current) => ({ ...current, [key]: current[key]?.source === "Recommended Starting Parameters" || current[key]?.source === "Smart Starting Point" ? { ...current[key]!, manuallyAdjusted: true } : { source: "Manual adjustment" } }));
  };
  const applyTrackedValues = (values: Array<{ key: RecommendedParameterKey; value: number }>, source: ChangeSource) => {
    for (const item of values) parameterSetters[item.key](item.value);
    setParameterTraces((current) => ({ ...current, ...Object.fromEntries(values.map((item) => [item.key, { source, previousValue: currentParameterValues[item.key], appliedValue: item.value }])) }));
  };
  const applySelectedRecommendations = () => {
    applyTrackedValues(initialRecommendation.parameters.filter((parameter): parameter is ParameterRecommendation & { value: number } => selectedRecommendationKeys.includes(parameter.key) && parameter.value !== undefined), "Recommended Starting Parameters");
    setProcessSearchExecuted(false);
    setSelectedHistoricalId("");
  ...
  const processMatches = useMemo(() => processSearchExecuted && executedProcessQuery ? searchSimilarProcessExperiments(contexts, executedProcessQuery, allSolutionMatches) : [], [contexts, executedProcessQuery, processSearchExecuted, allSolutionMatches]);
  ...
  const formulationReview = buildFormulationReview(formulation, materials);
  const characterizationReview = buildCharacterizationReview(characterization);
  const setupReview = buildSetupReview(setup);
  const operatingChanges = buildOperatingChanges(initialOperatingValues, currentParameterValues, parameterTraces);
  const operatingReview = compactRows((Object.keys(OPERATING_PARAMETER_META) as RecommendedParameterKey[]).map((key) => [OPERATING_PARAMETER_META[key].label, finiteNumberOrUndefined(currentParameterValues[key]) === undefined ? undefined : `${currentParameterValues[key]} ${OPERATING_PARAMETER_META[key].unit}`]));

  function applySmartStartingPoint() {
    applyTrackedValues(smartStartingPointChanges, "Smart Starting Point");
  ...
  const save = async () => {
    ...
    try {
      ...
      setRunName("");
      setComments("");
      setInitialOperatingValues(currentParameterValues);
      setParameterTraces({});
    } catch (caught) {
        ...
        {stage === "parameters" && (
          <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <TrackedNumericField parameterKey="flowRateMlH" label="Q1" unit="mL/h" value={flowRateMlH} trace={parameterTraces.flowRateMlH} onChange={(value) => updateOperatingParameter("flowRateMlH", value)} min={0} decimals={3} />
              <TrackedNumericField parameterKey="voltageKv" label="HV+" unit="kV" value={voltageKv} trace={parameterTraces.voltageKv} onChange={(value) => updateOperatingParameter("voltageKv", value)} decimals={2} />
              <TrackedNumericField parameterKey="collectorVoltageKv" label="HV-" unit="kV" value={collectorVoltageKv} trace={parameterTraces.collectorVoltageKv} onChange={(value) => updateOperatingParameter("collectorVoltageKv", value)} decimals={2} />
              <TrackedNumericField parameterKey="temperatureC" label="Temperature" unit="°C" value={temperatureC} trace={parameterTraces.temperatureC} onChange={(value) => updateOperatingParameter("temperatureC", value)} decimals={1} />
              <TrackedNumericField parameterKey="humidityPct" label="RH" unit="%" value={humidityPct} trace={parameterTraces.humidityPct} onChange={(value) => updateOperatingParameter("humidityPct", value)} min={0} max={100} decimals={1} />
              <TrackedNumericField parameterKey="distanceMm" label="dZ" unit="mm" value={distanceMm} trace={parameterTraces.distanceMm} onChange={(value) => updateOperatingParameter("distanceMm", value)} min={0} decimals={1} />
              <TrackedNumericField parameterKey="drumSpeedRpm" label="Drum speed" unit="rpm" value={drumSpeedRpm} trace={parameterTraces.drumSpeedRpm} onChange={(value) => updateOperatingParameter("drumSpeedRpm", value)} min={0} decimals={0} />
          ...
          <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid gap-4 lg:grid-cols-2">
              ...
              <OverviewCard title="Recommended Starting Parameters" subtitle="Get starting points from similar successful experiments." tone="violet" open={openAnalysisPanel === "recommendation"} onToggle={toggleRecommendationPanel}>
                ...
                <p className="mt-3 text-sm text-slate-700">Best Solution Similarity: <b>{initialRecommendation.bestMatch ? `${initialRecommendation.bestMatch.score.toFixed(0)}% Solution Similarity` : "No data"}</b></p>
        ...
        {stage === "review" && (
          <section className="mt-6 rounded-3xl border border-emerald-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-6 space-y-6">
              <ReviewSection title="Project"><ReviewRows rows={compactRows([["Project name / code", project.name], ["Description", project.description]])} /></ReviewSection>
              <ReviewSection title="Formulation"><ReviewRows rows={[{ label: "Formulation name", value: formulation.name || formulation.polymerName }, ...formulationReview.polymers, ...formulationReview.solvents, ...compactRows([["Notes", formulation.notes]])]} /></ReviewSection>
              <ReviewSection title="Solution Characterization">{characterizationReview.length > 0 || characterization?.measuredAt || characterization?.notes ? <ReviewRows rows={[...characterizationReview, ...compactRows([["Measurement date", characterization?.measuredAt ? formatReviewDate(characterization.measuredAt) : undefined], ["Notes", characterization?.notes]])]} /> : <p className="text-sm text-slate-500">No current characterization measurements selected.</p>}</ReviewSection>
              <ReviewSection title="Machine Setup"><ReviewRows rows={setupReview} /></ReviewSection>
              <ReviewSection title="Operating Parameters"><ReviewRows rows={operatingReview} /></ReviewSection>
              <ReviewSection title="Changes Made During This Run">{operatingChanges.length > 0 ? <div className="space-y-2">{operatingChanges.map((change) => <p key={change.key} className="rounded-xl bg-amber-50 px-3 py-2 text-sm text-amber-950"><b>{change.label}:</b> {displayTrackedValue(change.original, change.unit)} → {displayTrackedValue(change.final, change.unit)} · {change.source}</p>)}</div> : <p className="text-sm text-slate-600">No operating parameters were changed from the initially entered values.</p>}</ReviewSection>
              <ReviewSection title="Result"><ReviewRows rows={compactRows([["Run / sample code", runName || "Missing"], ["Observed processability grade", `${processability} / 4`], ["Process comments", comments]])} /></ReviewSection>
            </div>
...
function ReviewSection({ title, children }: { title: string; children: React.ReactNode }) { return <section className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4"><h3 className="font-bold text-slate-950">{title}</h3><div className="mt-3">{children}</div></section>; }
function ReviewRows({ rows }: { rows: Array<{ label: string; value: string }> }) { return <dl className="divide-y divide-slate-200">{rows.map((row) => <div key={`${row.label}-${row.value}`} className="grid min-w-0 gap-1 py-2 first:pt-0 last:pb-0 sm:grid-cols-[13rem_minmax(0,1fr)] sm:gap-4"><dt className="text-xs font-bold text-slate-500">{row.label}</dt><dd className="break-words text-sm text-slate-900">{row.value}</dd></div>)}</dl>; }
...
function TrackedNumericField({ parameterKey, trace, label, unit, value, onChange, min, max, decimals }: { parameterKey: RecommendedParameterKey; trace?: ParameterTrace; label: string; unit: string; value?: number; onChange: (value: number | undefined) => void; min?: number; max?: number; decimals?: number }) {
  const applied = trace?.source === "Recommended Starting Parameters" || trace?.source === "Smart Starting Point";
  return <div className="min-w-0"><NumericField label={label} unit={unit} value={value} onChange={onChange} min={min} max={max} decimals={decimals} />{applied && <div className={`mt-2 rounded-lg px-2.5 py-2 text-[10px] ${trace.manuallyAdjusted ? "bg-amber-50 text-amber-800" : "bg-violet-50 text-violet-800"}`}><p className="font-bold">{trace.manuallyAdjusted ? "Adjusted manually after recommendation" : trace.source === "Recommended Starting Parameters" ? "Applied from historical recommendation" : "Applied from Smart Starting Point"}</p><p className="mt-0.5">Previous: {displayTrackedValue(trace.previousValue, unit)} → Current: {displayTrackedValue(value, unit)}</p></div>}</div>;
}
function displayTrackedValue(value: number | undefined, unit: string): string { return value === undefined || !Number.isFinite(value) ? "No data" : `${value} ${unit}`; }
function formatReviewDate(value: string): string { const date = new Date(value); return Number.isNaN(date.getTime()) ? value : date.toLocaleDateString(); }
...
function ExperimentComparison({ current, match, processMatch }: { current: CurrentRunSnapshot; match: SolutionSimilarityMatch; processMatch?: import("../features/experimental-assistant/processConditionSimilarity.types").ProcessConditionMatch }) {
  ...
  const processCurrentValue = (key: ProcessConditionKey, liveValue: number | undefined) => processMatch?.selectedCriteria.includes(key) ? processMatch.queryValues[key] : liveValue;
  const processSelected = (key: ProcessConditionKey) => !processMatch || processMatch.selectedCriteria.includes(key);
  const rows = [
    ...
    processRow("Flow Rate", processCurrentValue("flowRateMlH", current.flowRateMlH), telemetry?.flowRateMlH, "flowRateMlH", processSelected("flowRateMlH")),
    processRow("HV+", processCurrentValue("voltageKv", current.voltageKv), telemetry?.voltageKv, "voltageKv", processSelected("voltageKv")),
    processRow("HV-", processCurrentValue("collectorVoltageKv", current.collectorVoltageKv), telemetry?.collectorVoltageKv, "collectorVoltageKv", processSelected("collectorVoltageKv")),
    processRow("Temperature", processCurrentValue("temperatureC", current.temperatureC), telemetry?.temperatureC, "temperatureC", processSelected("temperatureC")),
    processRow("Relative Humidity", processCurrentValue("humidityPct", current.humidityPct), telemetry?.humidityPct, "humidityPct", processSelected("humidityPct")),
    processRow("Working Distance", processCurrentValue("distanceMm", current.distanceMm), telemetry?.distanceMm, "distanceMm", processSelected("distanceMm")),
    processRow("Drum / Collector Speed", processCurrentValue("drumSpeedRpm", current.drumSpeedRpm), telemetry?.drumSpeedRpm, "drumSpeedRpm", processSelected("drumSpeedRpm")),
...
function InitialRecommendationPanel({ recommendation, selectedKeys, onSelectedKeysChange, currentValues, previewOpen, onPreviewOpen, onPreviewCancel, onApply, successMessage }: { recommendation: InitialParameterRecommendation; selectedKeys: RecommendedParameterKey[]; onSelectedKeysChange: (keys: RecommendedParameterKey[]) => void; currentValues: Partial<Record<RecommendedParameterKey, number>>; previewOpen: boolean; onPreviewOpen: () => void; onPreviewCancel: () => void; onApply: () => void; successMessage: string }) {
  ...
  const evidenceLevelText = recommendation.evidenceLevel === "insufficient" ? "Insufficient" : `${recommendation.evidenceLevel[0].toUpperCase()}${recommendation.evidenceLevel.slice(1)}`;
  const evidenceLabel = `Historical evidence level: ${evidenceLevelText}`;
  ...
  const unavailableParameters = recommendation.parameters.filter((parameter) => parameter.value === undefined);
  const successfulSupportingCount = new Set(recommendation.parameters.filter((parameter) => parameter.value !== undefined).flatMap((parameter) => parameter.sources.filter((source) => source.status === "included" && source.grade !== undefined && source.grade >= 3).map((source) => source.experimentId))).size;
  const excludedSourceCount = recommendation.parameters.reduce((count, parameter) => count + parameter.sources.filter((source) => source.status !== "included").length, 0);
  return <section className="rounded-3xl border border-violet-200 bg-violet-50/60 p-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><h3 className="text-lg font-bold text-slate-950">Recommended Starting Parameters</h3><p className="mt-1 text-xs text-slate-600">Historical starting points—not guaranteed machine setpoints.</p></div>
      <EvidenceBadge level={recommendation.evidenceLevel} label={evidenceLabel} />
    </div>
    <dl className="mt-4 grid gap-x-6 gap-y-3 rounded-2xl border border-violet-100 bg-white p-4 text-xs sm:grid-cols-2 lg:grid-cols-4">
      <SummaryMetric label="Similar experiments found" value={String(recommendation.supportingExperimentCount)} />
      <SummaryMetric label="Successful experiments supporting recommendations" value={String(successfulSupportingCount)} />
      <SummaryMetric label="Best Solution Similarity" value={recommendation.bestMatch ? `${formatRecommendationNumber(recommendation.bestMatch.score, 0)}% Solution Similarity` : "No data"} />
      <SummaryMetric label="Historical grade" value={recommendation.bestMatch && validGrade(recommendation.bestMatch.context.experiment.jetStabilityGrade) ? `${recommendation.bestMatch.context.experiment.jetStabilityGrade}/4` : "No data"} />
    </dl>
    <button type="button" aria-expanded={methodOpen} aria-controls="recommendation-method-explanation" onClick={() => setMethodOpen((open) => !open)} className="mt-4 rounded-xl border border-violet-200 bg-white px-4 py-2 text-left text-xs font-bold text-violet-700 outline-none hover:bg-violet-50 focus-visible:ring-2 focus-visible:ring-violet-500 focus-visible:ring-offset-2">How were these recommendations calculated? {methodOpen ? "▴" : "▾"}</button>
    {methodOpen && <div id="recommendation-method-explanation" className="mt-3 rounded-2xl border border-violet-200 bg-white p-4 text-xs text-slate-700"><ul className="list-disc space-y-1.5 pl-5"><li>Only sufficiently similar historical experiments are considered.</li><li>Successful experiments contribute more according to the existing calculation.</li><li>Missing values, outliers, and values without consensus are excluded.</li><li>No value is invented.</li></ul><p className="mt-3 font-semibold">{recommendation.supportingExperimentCount} similar experiments considered · {successfulSupportingCount} successful experiments supported at least one recommendation · {excludedSourceCount} parameter values excluded</p></div>}
    {reliableKeys.length > 0 ? <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><RecommendationParameterCards recommendation={recommendation} selectedKeys={selectedKeys} onSelectedKeysChange={onSelectedKeysChange} evidenceKey={evidenceKey} onEvidenceToggle={(key) => setEvidenceKey((current) => current === key ? null : key)} /></div> : <p className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 text-sm font-semibold text-slate-700">No reliable historical recommendations are available for the operating parameters.</p>}
    {unavailableParameters.length > 0 && reliableKeys.length > 0 && <p className="mt-3 text-xs text-slate-600">No reliable recommendation for: {unavailableParameters.map((parameter, index) => <React.Fragment key={parameter.key}>{index > 0 && ", "}<button type="button" aria-expanded={evidenceKey === parameter.key} aria-controls={`recommendation-sources-${parameter.key}`} onClick={() => setEvidenceKey((current) => current === parameter.key ? null : parameter.key)} className="font-bold text-violet-700 underline decoration-violet-300 underline-offset-2">{parameter.label}</button></React.Fragment>)}.</p>}
    {evidenceParameter && <RecommendationEvidencePanel parameter={evidenceParameter} />}
    {reliableKeys.length > 0 && <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-violet-200 pt-4"><label className="flex cursor-pointer items-center gap-2 text-xs font-semibold text-slate-700"><input type="checkbox" checked={allReliableSelected} onChange={(event) => onSelectedKeysChange(event.target.checked ? reliableKeys : [])} />Select all reliable recommendations</label><button type="button" disabled={selectedParameters.length === 0} onClick={onPreviewOpen} className="rounded-xl bg-violet-600 px-4 py-2.5 text-xs font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">Use selected starting parameters</button></div>}
    {previewOpen && <div role="dialog" aria-modal="true" aria-labelledby="recommendation-preview-title" className="mt-4 rounded-2xl border border-violet-300 bg-white p-4 shadow-sm"><h4 id="recommendation-preview-title" className="font-bold text-slate-950">Confirm starting parameters</h4><p className="mt-1 text-xs text-slate-600">Review every current value before replacing it in this unsaved run.</p><div className="mt-3 overflow-x-auto"><table className="w-full min-w-[480px] text-left text-xs"><thead><tr className="border-b border-slate-200 text-slate-500"><th className="px-2 py-2">Parameter</th><th className="px-2 py-2">Current value</th><th className="px-2 py-2">Proposed value</th></tr></thead><tbody>{selectedParameters.map((parameter) => <tr key={parameter.key} className="border-b border-slate-100"><td className="px-2 py-2 font-semibold">{parameter.label}</td><td className="px-2 py-2">{formatRecommendationValue(parameter.key, currentValues[parameter.key])} {parameter.unit}</td><td className="px-2 py-2 font-bold text-violet-800">{formatRecommendationValue(parameter.key, parameter.value!)} {parameter.unit}</td></tr>)}</tbody></table></div><div className="mt-4 flex justify-end gap-2"><button type="button" onClick={onPreviewCancel} className="rounded-xl border border-slate-300 px-4 py-2 text-xs font-bold text-slate-700">Cancel</button><button type="button" onClick={onApply} className="rounded-xl bg-violet-600 px-4 py-2 text-xs font-bold text-white">Confirm and copy</button></div></div>}
    {successMessage && <p role="status" className="mt-4 rounded-xl bg-emerald-50 p-3 text-xs font-semibold text-emerald-800">{successMessage}</p>}
  </section>;
}

function SummaryMetric({ label, value }: { label: string; value: string }) { return <div className="min-w-0"><dt className="text-[10px] font-bold uppercase tracking-wider text-slate-500">{label}</dt><dd className="mt-1 break-words font-semibold text-slate-900">{value}</dd></div>; }
...
function RecommendationParameterCards({ recommendation, selectedKeys, onSelectedKeysChange, evidenceKey, onEvidenceToggle }: { recommendation: InitialParameterRecommendation; selectedKeys: RecommendedParameterKey[]; onSelectedKeysChange: (keys: RecommendedParameterKey[]) => void; evidenceKey: RecommendedParameterKey | null; onEvidenceToggle: (key: RecommendedParameterKey) => void }) {
  return <>{recommendation.parameters.filter((parameter) => parameter.value !== undefined).map((parameter) => <RecommendationParameterCard key={parameter.key} parameter={parameter} selected={selectedKeys.includes(parameter.key)} onSelectedChange={(selected) => onSelectedKeysChange(selected ? [...selectedKeys, parameter.key] : selectedKeys.filter((key) => key !== parameter.key))} evidenceOpen={evidenceKey === parameter.key} onEvidenceToggle={() => onEvidenceToggle(parameter.key)} />)}</>;
}
...
function RecommendationParameterCard({ parameter, selected, onSelectedChange, evidenceOpen, onEvidenceToggle }: { parameter: ParameterRecommendation; selected: boolean; onSelectedChange: (selected: boolean) => void; evidenceOpen: boolean; onEvidenceToggle: () => void }) {
  ...
  return <article className="flex min-w-0 flex-col rounded-2xl border border-violet-200 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-2"><p className="text-sm font-bold text-slate-900">{parameter.label}</p><EvidenceBadge level={parameter.evidenceLevel} /></div><p className="mt-3 text-lg font-bold text-slate-950">{reliable ? <>{formatRecommendationValue(parameter.key, parameter.value!)} <span className="text-sm font-semibold">{parameter.unit}</span></> : "No reliable historical recommendation"}</p>{reliable ? <div className="mt-3 space-y-1 text-xs text-slate-600"><p>Based on <b>{parameter.successfulExperimentCount}</b> successful experiment{parameter.successfulExperimentCount === 1 ? "" : "s"}</p>{parameter.range && <p>Historical usable range: <b>{formatRecommendationValue(parameter.key, parameter.range.minimum)}–{formatRecommendationValue(parameter.key, parameter.range.maximum)} {parameter.unit}</b></p>}</div> : <p className="mt-2 text-xs text-slate-600">{noRecommendationReason(parameter)}</p>}{reliable && <label className="mt-3 flex cursor-pointer items-start gap-2 text-xs font-semibold text-slate-700"><input type="checkbox" checked={selected} onChange={(event) => onSelectedChange(event.target.checked)} aria-label={`Apply ${parameter.label} recommendation`} className="mt-0.5" />Apply this recommendation</label>}<button type="button" aria-expanded={evidenceOpen} aria-controls={disclosureId} onClick={onEvidenceToggle} className="mt-auto pt-4 text-left text-xs font-bold text-violet-700 outline-none focus-visible:ring-2 focus-visible:ring-violet-500">Why this recommendation? {evidenceOpen ? "▴" : "▾"}</button></article>;
}
...
function RecommendationEvidencePanel({ parameter }: { parameter: ParameterRecommendation }) {
  ...
  return <section id={`recommendation-sources-${parameter.key}`} className="mt-4 w-full rounded-2xl border border-violet-200 bg-white p-4"><h4 className="font-bold text-slate-950">Why {parameter.label} was recommended</h4><p className="mt-1 text-xs text-slate-600">{included.length} included value{included.length === 1 ? "" : "s"} · {excluded.length} excluded value{excluded.length === 1 ? "" : "s"}</p><div className="mt-4 grid gap-4 lg:grid-cols-2"><RecommendationSourceGroup title="Included supporting experiments" sources={included} parameter={parameter} /><RecommendationSourceGroup title="Excluded historical values" sources={excluded} parameter={parameter} /></div></section>;
}
...
function RecommendationSourceGroup({ title, sources, parameter }: { title: string; sources: ParameterRecommendation["sources"]; parameter: ParameterRecommendation }) {
  return <section className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">{title}</p>{sources.length === 0 ? <p className="mt-2 text-xs text-slate-500">None.</p> : <div className="mt-2 space-y-2">{sources.map((source, index) => <article key={`${parameter.key}-${source.experimentId}-${source.status}`} className="min-w-0 rounded-xl bg-slate-50 p-3 text-[11px] text-slate-600"><div className="flex flex-wrap items-start justify-between gap-2"><p className="min-w-0 flex-1 break-words font-bold text-slate-800">{readableExperimentName(source.experimentName, source.experimentId, index)}</p><span className={`rounded-full px-2 py-0.5 font-bold ${source.status === "included" ? "bg-emerald-100 text-emerald-700" : source.status === "outlier" ? "bg-amber-100 text-amber-800" : source.status === "no-consensus" ? "bg-red-100 text-red-700" : "bg-slate-200 text-slate-700"}`}>{source.status === "included" ? "Included" : source.status === "outlier" ? "Excluded as outlier" : source.status === "no-consensus" ? "Excluded: no parameter consensus" : "Excluded: insufficient solution evidence"}</span></div><p className="mt-2">Raw value: <b>{formatRecommendationValue(parameter.key, source.rawValue)} {source.unit}</b></p><p>Solution Similarity: {formatRecommendationNumber(source.solutionSimilarity, 0)}% · Grade: {source.grade !== undefined ? `${source.grade}/4` : "No data"}</p>{source.exclusionReason && <p className="mt-1">{source.exclusionReason}</p>}<details className="mt-2"><summary className="cursor-pointer font-bold text-violet-700">Calculation details</summary><p className="mt-1">Success weight: {formatRecommendationNumber(source.successWeight, 2)} · Contribution weight: {formatRecommendationNumber(source.contributionWeight, 3)}</p></details></article>)}</div>}</section>;
}

function noRecommendationReason(parameter: ParameterRecommendation): string {
  if (parameter.sources.length === 0) return "Historical values are missing for this parameter.";
  if (parameter.sources.some((source) => source.status === "no-consensus")) return "Historical values did not reach a consistent parameter consensus.";
  if (parameter.supportingExperimentCount < 2) return "Fewer than two supporting experiments were available.";
  return "The available historical evidence was not reliable enough for a starting value.";
}

function readableExperimentName(name: string, id: string, index: number): string {
  const normalized = name.trim();
  const looksTechnical = !normalized || normalized === id || /^(?:EXP|XLS|SOL|RUN)?[_-]?[0-9a-f]{8}(?:[-_][0-9a-f]{4}){3}[-_][0-9a-f]{12}$/i.test(normalized) || /^(?:EXP|XLS)[_-]/i.test(normalized);
  return looksTechnical ? `Historical experiment ${index + 1}` : normalized;
}

...
import { hasFiniteCharacterizationMeasurement, parseOptionalMeasurement } from "../features/characterizations/characterizationMeasurements";
import {
...
export default function Formulations({
  ...
  const parsedCharacterizationMeasurements = {
    solidsContentPct: parseOptionalMeasurement(charForm.solidsContentPct),
    viscosityMpas: parseOptionalMeasurement(charForm.viscosityMpas),
    conductivityUsCm: parseOptionalMeasurement(charForm.conductivityUsCm),
    densityGcm3: parseOptionalMeasurement(charForm.densityGcm3),
    surfaceTensionMnM: parseOptionalMeasurement(charForm.surfaceTensionMnM),
    ph: parseOptionalMeasurement(charForm.ph),
  };
  const canSaveCharacterization = hasFiniteCharacterizationMeasurement(parsedCharacterizationMeasurements);
...
const createMaterial = async (
  ...
  const createCharacterization = async (event: React.FormEvent) => {
    ...
    try {
      await onAddCharacterization({
        ...
        ...parsedCharacterizationMeasurements,
  ...
  <section role="dialog" aria-modal="false" aria-labelledby="polymer-catalog-title" onKeyDown={(event) => { if (event.key === "Escape" && !materialSaving) closePolymerCatalog(); }} className="rounded-2xl border border-blue-200 bg-blue-50 p-5">
    ...
    <p className="mt-3 text-xs text-slate-600">Category: Polymer. No formulation is created by this action.</p>
              ...
              <div className="min-w-0 rounded-2xl border border-indigo-100 bg-gradient-to-br from-blue-50/60 to-violet-50/70 p-4 sm:p-5">
                ...
                <div className="mt-3 space-y-4">
                  {optionalPolymerRowCount >= 1 && (
                    <div className="relative min-w-0 overflow-hidden rounded-xl border border-indigo-100 bg-white p-4 pt-12 shadow-sm shadow-indigo-100/60 [&_.input]:bg-white">
                      <button type="button" aria-label="Remove Polymer 2" onClick={() => removePolymerRow(2)} className="absolute right-3 top-3 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700 outline-none transition-colors hover:border-indigo-300 hover:bg-indigo-100 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2">
                        × Remove
                      </button>
                  ...
                  {optionalPolymerRowCount >= 2 && (
                    <div className="relative min-w-0 overflow-hidden rounded-xl border border-indigo-100 bg-white p-4 pt-12 shadow-sm shadow-indigo-100/60 [&_.input]:bg-white">
                      <button type="button" aria-label="Remove Polymer 3" onClick={() => removePolymerRow(3)} className="absolute right-3 top-3 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-bold text-indigo-700 outline-none transition-colors hover:border-indigo-300 hover:bg-indigo-100 focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2">
                        × Remove
                      </button>
            ...
            <section className="mt-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              {showCharacterization && (
                <form onSubmit={createCharacterization} className="mt-5 border-t border-slate-100 pt-5">
                  ...
                  {!canSaveCharacterization && <p className="mt-3 text-sm font-semibold text-amber-700">Enter at least one measurement before saving.</p>}
                  ...
                  <button disabled={saving || !canSaveCharacterization} className="mt-4 w-full rounded-2xl bg-violet-600 px-5 py-3 font-bold text-white disabled:opacity-50">Save Characterization</button>
                </form>
              )}
            </section>
